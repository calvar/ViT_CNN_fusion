...
# CNN Backbone using EfficientNet-B0
class CNNBackbone(nn.Module):
    def __init__(self, embed_dim=256):
        super(CNNBackbone, self).__init__()
        efftnet = models.efficientnet_b0(weights='DEFAULT')
        self.features = efftnet.features  # Extract the feature layers
        # Freeze the feature extraction layers
        for param in self.features.parameters():
            param.requires_grad = False
        self.dense = nn.Linear(1280, embed_dim)  # Project to embed_dim
        
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.features(x)
        x = torch.flatten(x, start_dim=2) # Flatten the last two dimensions of the output
        x = torch.permute(x, (0, 2, 1))  # Change shape to (B, 7x7, 1280)
        x = self.dense(x) # Project to embed_dim
        return x  # (B, 7x7, embed_dim)
...
# Vision Transformer x CNN Model
class TokenFusionCnnVit(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_channels=3, num_classes=10,
                 embed_dim=256, num_heads=8, depth=6, mlp_dim=512, drop_rate=0.1):
        super(TokenFusionCnnVit, self).__init__()
        self.patch_embed = PatchEmbedding(img_size, patch_size, in_channels, embed_dim)
        self.cnn_backbone = CNNBackbone(embed_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x1 = self.patch_embed(x)  # (B, num_patches + 1, embed_dim)
        x2 = self.cnn_backbone(x)  # (B, 7x7, embed_dim)
        x = torch.cat((x1, x2), dim=1)  # Fuse the features
        x = self.encoder(x)
        x = self.norm(x)
